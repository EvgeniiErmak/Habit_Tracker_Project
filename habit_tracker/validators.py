...
def validate_reward_and_related_habit(value, related_habit):
    if value and related_habit:
        raise ValidationError("Привычка не может иметь и вознаграждение, и связанную привычку одновременно.")
...
def validate_related_habit(value, pleasant):
    if value and not pleasant:
        raise ValidationError("Связанную привычку можно указывать только для приятных привычек.")
