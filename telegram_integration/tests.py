...
class TelegramIntegrationViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.user_profile = UserProfile.objects.create(
            user=self.user, telegram_chat_id=12345
        )
    ...
    def test_habits_command_without_user_profile(self):
        UserProfile.objects.filter(
            user=self.user
        ).delete()  # Delete user profile for the test
        update = Update(message=MockChat(id=12345), effective_user=MockUser(id=12345))
        response = habits_command(update, None)
        expected_response = (
            "Ваш профиль не найден. Пожалуйста, начните с команды /start."
        )
        self.assertEqual(response, expected_response)
...
class TelegramBotTestCase(TestCase):
    ...
    def test_start_bot(self, mock_updater, mock_bot):
        ...
        mock_updater.assert_called_once_with(
            bot=mock_bot.return_value, use_context=True
        )
    ...
    def test_start_handler(self):
        ...
        context.bot.send_message.assert_called_once_with(
            chat_id=12345, text="Bot started!"
        )
    ...
    def test_habits_handler(self):
        ...
        context.bot.send_message.assert_called_once_with(
            chat_id=12345, text="List of your habits:\n1. Habit 1\n2. Habit 2"
        )
    ...
    def test_add_habit_handler(self):
        ...
        context.bot.send_message.assert_called_once_with(
            chat_id=12345, text="Enter the name of the new habit"
        )
    ...
    def test_echo_handler(self):
        ...
        context.bot.send_message.assert_called_once_with(
            chat_id=12345, text="You said: Test Message"
        )
    ...
    def test_error_handler(self, mock_logger):
        ...
        mock_logger.error.assert_called_once_with(
            f"Update {update} caused error Test Error"
        )
...
class TelegramIntegrationTestCase(TestCase):
    ...
    def test_send_reminders(self, mock_telegram_bot):
        ...
        habit = Habit.objects.create(
            user=self.user, name="Test Habit", place="Home", time=current_time
        )
        send_reminders()
    ...
    def test_send_habit_reminder(self, mock_telegram_bot):
        habit = Habit.objects.create(
            user=self.user, name="Test Habit", place="Home", time=timezone.now().time()
        )
        habit_id = habit.id
